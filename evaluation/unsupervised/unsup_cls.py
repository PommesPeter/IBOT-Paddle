...
def main_eval(args):
    ...
    data_loader_val = paddle.utils.data.DataLoader(
        dataset_val,
    ...
    if "swin" in args.arch:
        ...
        model = linear.__dict__[args.arch](patch_size=args.patch_size, num_classes=0)
        embed_dim = model.embed_dim
    print(f"Model {args.arch} {args.patch_size}x{args.patch_size} built.")
    model = utils.MultiCropWrapper(model, DINOHead(embed_dim, args.out_dim, act="gelu"))
    model.cuda(args.local_rank)

    model = paddle.nn.parallel.DistributedDataParallel(
        model, device_ids=[args.local_rank]
    )
    utils.restart_from_checkpoint(
        args.pretrained_weights, **{args.checkpoint_key: model}
    )
    model.eval()
