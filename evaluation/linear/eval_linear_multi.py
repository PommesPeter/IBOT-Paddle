...
def eval_linear(args):
    ...
    sampler = paddle.utils.data.distributed.DistributedSampler(dataset_train)
    train_loader = paddle.utils.data.DataLoader(
        dataset_train,
        sampler=sampler,
    ...
    val_loader = paddle.utils.data.DataLoader(
        dataset_val,
        batch_size=args.batch_size_per_gpu,
        num_workers=args.num_workers,
        pin_memory=True,
    ...
    if 'swin' in args.arch:
        args.patch_size = 4
        model = linear.__dict__[args.arch](
    ...
    for pm in args.permutes:
        ...
        optimizer = paddle.optim.SGD if optim == 'sgd' else utils.LARS
        optimizer = optimizer(
            parameters,
        ...
        scheduler = paddle.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min=0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            if utils.is_main_process() and (group_best_acc >= best_acc):
                ...
                paddle.save(save_dict, os.path.join(args.output_dir, "checkpoint_{}_linear.pth".format(args.checkpoint_key)))
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        with paddle.no_grad():
            ...
            if avgpool == 0:
                ...
                output = [x[:, 0] for x in intermediate_output] + [paddle.mean(intermediate_output[-1][:, 1:], dim=1)]
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dist_url", default="env://", type=str, help="""url used to set up
        distributed training; see https://pypaddle.org/docs/stable/distributed.html""")
